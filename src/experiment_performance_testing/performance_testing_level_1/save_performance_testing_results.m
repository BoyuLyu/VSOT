function save_performance_testing_results(data_path, quantification_save_path)
    ...
    fprintf('VSOT: F1 %.2f, precision %.2f,recall %.2f, Jeccard %.2f \n', ...
        mean(us_score(:,1)), mean(us_score(:,2)), mean(us_score(:,3)),mean(us_score(:,4)));
    fprintf('Neurd: F1 %.2f, precision %.2f,recall %.2f, Jeccard %.2f \n', ...
        mean(neurd_score(:,1)), mean(neurd_score(:,2)), mean(neurd_score(:,3)),mean(neurd_score(:,4)));
    fprintf('Spinetool: F1 %.2f, precision %.2f,recall %.2f, Jeccard %.2f \n', ...
        mean(spinetool_score(:,1)), mean(spinetool_score(:,2)), mean(spinetool_score(:,3)),mean(spinetool_score(:,4)));
    fprintf('Morph: F1 %.2f, precision %.2f,recall %.2f, Jeccard %.2f \n', ...
        mean(morph_score(:,1)), mean(morph_score(:,2)), mean(morph_score(:,3)),mean(morph_score(:,4)));
    
    fprintf('VSOT: F1 %.2f, precision %.2f,recall %.2f, Jeccard %.2f \n', ...
        std(us_score(:,1)), std(us_score(:,2)), std(us_score(:,3)),std(us_score(:,4)));
    fprintf('Neurd: F1 %.2f, precision %.2f,recall %.2f, Jeccard %.2f \n', ...
        std(neurd_score(:,1)), std(neurd_score(:,2)), std(neurd_score(:,3)),std(neurd_score(:,4)));
    fprintf('Spinetool: F1 %.2f, precision %.2f,recall %.2f, Jeccard %.2f \n', ...
        std(spinetool_score(:,1)), std(spinetool_score(:,2)), std(spinetool_score(:,3)),std(spinetool_score(:,4)));
    fprintf('Morph: F1 %.2f, precision %.2f,recall %.2f, Jeccard %.2f \n', ...
        std(morph_score(:,1)), std(morph_score(:,2)), std(morph_score(:,3)),std(morph_score(:,4)));
