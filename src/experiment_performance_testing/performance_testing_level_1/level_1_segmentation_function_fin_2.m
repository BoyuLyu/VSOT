function out_dendrite = level_1_segmentation_function_fin_2(mask_dendrite,skel_x,dist_dendrite, resx, resy, resz,lambda)
    % dist_dendrite is the result of the edt_mex
    % the mask_dendrite and the skel_x are the volumes of the dendritic
    % branch and the skeleton obtained from Kimimaro
    % resx, resy, resz are the resolution of the three dimensions (nm)
...
%     addpath('/home/boyu/Documents/src_mex/mex_EM_analysis/mex_EM_analysis')
    ...
    radius = min(distLine(distLine > min(resx, resz)));
    ...
    nei6 = regionGrow3D_conn6(ccID, lenx, leny, lenz);
    mask_target_whole_Area1d = logical(mask_dendrite(:));
    ...
    graph_mat_2 = [[source_cc(:);rest_cc(:)],repmat(psSourceID, length(source_cc) + length(rest_cc), 1)]; % link to source, this relates to the volume of the final extracted shaft region
    score_graph_2 = [inf(length(source_cc), 1); ones(length(rest_cc),1)];
    % lambda = 1/4;
    score_graph_2 = score_graph_2*(resx*resy*resz)^(2/3)/scale_factor*lambda; % (resx*resy*resz)^(2/3) is the average surface area of a voxel
    ...
    graph_matx((ismember(graph_matx(:,1), source_cc) & ismember(graph_matx(:,2), sink_cc))...
     | (ismember(graph_matx(:,2), source_cc) & ismember(graph_matx(:,1), sink_cc)),:) = []; % cut any direct connections between the source and the sink
