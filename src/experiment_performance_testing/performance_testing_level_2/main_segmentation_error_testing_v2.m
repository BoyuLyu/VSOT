...
% clear
function out = main_segmentation_error_testing_v2(offFolder,annotationFolder_gt_curves,tamada_result_folder,our_method_cut_result_folder,ofer_output_folder,dorkenwalk_result_folder,save_qauntification_folder)
    ...
    tamada_error_iou = tamada_error_all;
    ...
    our_method_error_iou = our_method_error_all;
    ...
    ofer_method_error_iou = ofer_method_error_all;
    ...
    dorkenwalk_error_iou = dorkenwalk_error_all;
    for j = 1:length(listx)
        ...
        if(exist(fullfile(annotationFolder_gt_curves, [namex,'.cut.txt']), 'file'))
            ...
            if(exist(fullfile(tamada_result_folder, [namex,'.cut.txt']), 'file'))
                ...
                [error_tamda_tmp, error_tamda_tmp_dist, iou_tamada] = checkSegDiff_v2(Tri, Pts, face_gt, face_label,edge_graph, gt_cut, tamada_cut);
                tamada_error_all(j) = error_tamda_tmp;
                tamada_error_all_dist(j) = error_tamda_tmp_dist;
                tamada_error_iou(j) = iou_tamada;
            end
            ...
            if(exist(fullfile(our_method_cut_result_folder,[namex,'.cut.txt']), 'file'))
                ...
                face_label = face_label.head_neck_label;
                face_label = face_label(:);
                [error_our_tmp,error_our_tmp_dist, iou_our]  = checkSegDiff_v2(Tri, Pts, face_gt, face_label, edge_graph, gt_cut, our_method_cut);
                our_method_error_all(j) = error_our_tmp;
                our_method_error_all_dist(j) = error_our_tmp_dist;
                our_method_error_iou(j) = iou_our;
            end
            catch ME
                continue;
            end
            try 
                if(exist(fullfile(ofer_output_folder,[namex,'.cut.txt']), 'file'))
                    ...
                    face_label = face_label.head_neck_label;
                    face_label = face_label(:);
                    [error_ofer_tmp,error_ofer_tmp_dist, iou_ofer]= checkSegDiff_v2(Tri, Pts, face_gt, face_label, edge_graph, gt_cut, ofer_method_cut);
                    ofer_method_error_all(j) = error_ofer_tmp;
                    ofer_method_error_all_dist(j) = error_ofer_tmp_dist;
                    ofer_method_error_iou(j) = iou_ofer;
                end
            catch ME
                continue;
            end
            try 
                if(exist(fullfile(dorkenwalk_result_folder,[namex,'.cut.txt']), 'file'))
                    ...
                    face_label = face_label.head_neck_label;
                    face_label = face_label(:);
                    [error_dorkenwald_tmp,error_dorkenwald_tmp_dist, iou_dorkenwald] = checkSegDiff_v2(Tri, Pts, face_gt, face_label,edge_graph, gt_cut, dorkenwald_method_cut);
                    dorkenwalk_error_all(j) = error_dorkenwald_tmp;
                    dorkenwalk_error_all_dist(j) = error_dorkenwald_tmp_dist;
                    dorkenwalk_error_iou(j) = iou_dorkenwald;
                end
            catch ME
                continue;
            end
