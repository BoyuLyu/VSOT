...
% clear
function main_segmentation_error_testing(offFolder,annotationFolder, annotationFolder_gt_curves,tamada_result_folder,our_method_cut_result_folder,ofer_output_folder,dorkenwalk_result_folder,save_qauntification_folder)
    ...
    for j = 1:length(listx)
        ...
        if(exist(fullfile(tamada_result_folder, [namex,'_cut.txt']), 'file'))
            tamada_cut = readtable(fullfile(tamada_result_folder, [namex,'_cut.txt']));
        ...
        if(exist(fullfile(ofer_output_folder,[namex,'_cut.txt']), 'file'))

            ofer_method_cut = readtable(fullfile(ofer_output_folder,[namex,'_cut.txt']));
        ...
        if(exist(fullfile(dorkenwalk_result_folder,[namex,'_cut.txt']), 'file'))

            dorkenwald_method_cut = readtable(fullfile(dorkenwalk_result_folder,[namex,'_cut.txt']));
