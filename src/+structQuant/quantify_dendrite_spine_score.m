function quantify_dendrite_spine_score(curpsID, lenx, leny, lenz,fullSegfolder_root,spine_save_folder, spine_head_neck_save_folder, matSaveFolder, resx, resy, resz)
...
doubleSynHeadVolume = zeros(length(spine_mask_coordinate_list),1);%d1
doubleSynMeanHeadRadius = zeros(length(spine_mask_coordinate_list),1);%d2
doubleSynNeckLength = zeros(length(spine_mask_coordinate_list),1);%d3
doubleSynNeckSection = zeros(length(spine_mask_coordinate_list),1);%d4
doubleSynNeckMeanRadius = zeros(length(spine_mask_coordinate_list),1);%d5
doubleSynapticCleftSize = zeros(length(spine_mask_coordinate_list),2);%d6
dousperimeterRatio = zeros(length(spine_mask_coordinate_list),2);%d7
dousperimeterWeightedWrappingArea = zeros(length(spine_mask_coordinate_list),2);%d8
douspostSynapseTouchingArea = zeros(length(spine_mask_coordinate_list),2);%d9
douspostSynapseTouchingRatio = zeros(length(spine_mask_coordinate_list),2);%d10
douspreSynapseTouchingArea = zeros(length(spine_mask_coordinate_list),2);%d11
douspreSynapseTouchingRatio = zeros(length(spine_mask_coordinate_list),2);%d12
doubleSynHeadNeckTouchingArea = zeros(length(spine_mask_coordinate_list),2);%d13
doubleSynHeadNeckTouchingRatio = zeros(length(spine_mask_coordinate_list),2);%d14
doubleSynPosition = zeros(length(spine_mask_coordinate_list),2);%d15
...
%                                 postSynNewID = datax.(curpsIDNew);
            ...
            for k = 1:length(id_selected)
                ...
                [single_features_all, double_features_all] = structQuant.gen_quantification_spine_sub_function(spine_mask,cleft_mask_small,astro_mask_small,seg_mask_small,datax_small(curpsIDNew), resx, resy, resz);
                if(isempty(fieldnames(single_features_all)) && ~isempty(fieldnames(double_features_all)))
                    synpatic_double_single_indicator(kk) = double_features_all.synpatic_double_single_indicator;
                    doubleSynHeadVolume(kk) = double_features_all.headVolumex;%d1
                    doubleSynMeanHeadRadius(kk) = double_features_all.headMeanRadiusx;%d2
                    doubleSynNeckLength(kk) = double_features_all.neckLengthx;%d3
                    doubleSynNeckSection(kk) = double_features_all.neckSectionx;%d4
                    doubleSynNeckMeanRadius(kk) = double_features_all.neckMeanRadiusx;%d5
                    doubleSynapticCleftSize(kk,:) = double_features_all.doubleSynapticCleftSize;%d6
                    dousperimeterRatio(kk,:) = double_features_all.dousperimeterRatio;%d7
                    dousperimeterWeightedWrappingArea(kk,:) = double_features_all.dousperimeterWeightedWrappingArea;%d8
                    douspostSynapseTouchingArea(kk,:) = double_features_all.douspostSynapseTouchingArea;%d9
                    douspostSynapseTouchingRatio(kk,:) = double_features_all.douspostSynapseTouchingRatio;%d10
                    douspreSynapseTouchingArea(kk,:) = double_features_all.douspreSynapseTouchingArea;%d11
                    douspreSynapseTouchingRatio(kk,:) = double_features_all.douspreSynapseTouchingRatio;%d12
                    doubleSynHeadNeckTouchingArea(kk,:) = double_features_all.doubleSynHeadNeckTouchingArea;%d13
                    doubleSynHeadNeckTouchingRatio(kk,:) = double_features_all.doubleSynHeadNeckTouchingRatio;%d14
                    doubleSynPosition(kk,:) = double_features_all.doubleSynPosition;%d15
                    spineCoordinate(kk,:) = double_features_all.spineCoordinate;
                        
                elseif(isempty(fieldnames(double_features_all)) && ~isempty(fieldnames(single_features_all)))
...
toc;
        ...
        summaryStructure.ds1 = doubleSynHeadVolume;
        summaryStructure.ds2 = doubleSynMeanHeadRadius;
        summaryStructure.ds3 = doubleSynNeckLength;
        summaryStructure.ds4 = doubleSynNeckSection;
        summaryStructure.ds5 = doubleSynNeckMeanRadius;
        summaryStructure.da1 = doubleSynapticCleftSize;
        summaryStructure.da2 = dousperimeterRatio;
        summaryStructure.da3 = dousperimeterWeightedWrappingArea;
        summaryStructure.da4 = douspostSynapseTouchingArea;
        summaryStructure.da5 = douspostSynapseTouchingRatio;
        summaryStructure.da6 = douspreSynapseTouchingArea;
        summaryStructure.da7 = douspreSynapseTouchingRatio;
        summaryStructure.da8 = doubleSynHeadNeckTouchingArea;
        summaryStructure.da9 = doubleSynHeadNeckTouchingRatio;
        summaryStructure.doubleSynPosition = doubleSynPosition;
        ...
        save(fullfile(matSaveFolder, 'summary.mat'), 'summaryStructure')
