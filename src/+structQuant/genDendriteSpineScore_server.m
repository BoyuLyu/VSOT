...
% addpath('/home/boyu/Documents/edt_mex/edt_mex/edt_mex')
    ...
    spineHNROIx = zeros(lenx, leny, lenz);
    spineHNROIx = spineHNROI;
    cleftx = zeros(lenx, leny, lenz);
    cleftx(cleftID) = 1;
    [allidx, allidy, allidz] = ind2sub([lenx, leny, lenz], [cleftID(:); find(spineHNROIx(:) > 0)]);
    ...
    cleftx = cleftx(bbx(1):bbx(2), bby(1):bby(2), bbz(1):bbz(2));
    cleftx = padarray(cleftx, [1,1,1], 0, 'both');
    ...
    if(sum(spineHNROIx(:) == 3) == 0 || sum(spineHNROIx(:) == 2) == 0)
        ...
        spineHNROI_neck = zeros(size(cleftx));
        singleSynHeadVolume = length(hid);
        se  = strel('sphere', 1);
    %     singleSynHeadMeanRadius = (singleSynHeadVolume*16*16*40/4*3/pi)^(1/3);
        cleftID = find(cleftx(:) == 1);
        [lenx, leny, lenz] = size(cleftx);
        [cleftIDx, cleftIDy,cleftIDz] = ind2sub([lenx, leny, lenz], cleftID);
        cleftIDxyz = [cleftIDx(:)*resx, cleftIDy(:)*resy,cleftIDz(:)*resz];
        [coeff,score_w,latent] = pca(cleftIDxyz);  
    
        [hidx, hidy, hidz] = ind2sub([lenx, leny, lenz], hid);
        hidxyz = [hidx(:)*resx, hidy(:)*resy, hidz(:)*resz];
