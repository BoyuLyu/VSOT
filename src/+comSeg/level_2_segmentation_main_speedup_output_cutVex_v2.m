function level_2_segmentation_main_speedup_output_cutVex_v2(spine_save_folder, spine_head_neck_save_folder, outputFolder, curvature_scale, resx, resy, resz,minpoints,smooth_kernel)
...
%                     
    %%=========================================================================================================
            ...
            contactIDxyz = [contactIDx(:)*resx, contactIDy(:)*resy,contactIDz(:)*resz];
            center_neck = mean(contactIDxyz, 1);
            dist_2_center_neck = vecnorm((Vnieuwx - center_neck),2,2);
            neckpoint = find(dist_2_center_neck == min(dist_2_center_neck),1);
            ...
            ccHead = find(distVexStartingPoint > quantile(distVexStartingPoint,0.99));
    ...
    % Calculate the curvature score using the adaptive scale
            ...
            [newCurvature] = comSeg.genCurvatureFace4Scale_speedup(mF2, Vnieuw2, curvature_scale, 1:size(Vnieuw2,1)); % 48 cores 2.16 s
    ...
    % Calculate the centerline width score for the cylinder map in the 
            ...
            non_neck = setdiff([1:size(Vnieuw,1)], ccNeck);
            test_region(sub2ind([lenxtmp, lenytmp, lenztmp], Vnieuw(:,1), Vnieuw(:,2), Vnieuw(:,3))) = 1;
            [pathidxyz, width_eachp] = comSeg.calWidthfromVol(test_region, Vnieuw(neckpoint,:), Vnieuw(headpoint,:),Vnieuwx(non_neck,:),resx, resy, resz); % width is from the tip of the shaft to the tip of the head
    ...
    %         real_id_point = comSeg.matchSScycle(Vnieuwx,cutVexcellUnique2(idSelected(coarse_cycle_path)), Vnieuw3, coarse_cycle_path);
            if(~isempty(id_point))
                ...
                tifwrite(uint8(spineHead), fullfile(spine_head_neck_save_folder, [files_x(k).name, '_head_neck']));
                whole_volume = imerode((head_volume*2 + neckx), se);
            else
                head_volume = test_region;
                % head_volume(contactID) = 0;
                % neckx = head_volume.*0;
                % neckx(contactID) = 1;
                spineHead = double(head_volume)*3 + double(shaft_mask);
                tifwrite(uint8(spineHead), fullfile(spine_head_neck_save_folder, [files_x(k).name, '_head_neck']));
                whole_volume = imerode((head_volume*2), se);
